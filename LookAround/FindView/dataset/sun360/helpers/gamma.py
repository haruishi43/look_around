...
def gather_image_paths_in_category(
    ...
    for subcat_name, subcat_path in subcat_paths.items():
        subcat_img_names = [
            i
            for i in os.listdir(subcat_path)
            if os.path.splitext(i)[-1] == ".jpg"
        ]
        subcat_img_paths = [
            os.path.relpath(os.path.join(subcat_path, i), data_root)
            for i in subcat_img_names
        ]
...
def create_splits_for_category(
    ...
    for _, subcat_img_paths in img_paths.items():
        ...
        if total == 1:
            ...
            if num_test < 1:
                val, test, train = [
                    list(arr) for arr in np.split(subcat_img_paths, [1, 2])
                ]
            else:
                split_indices = []
                index = 0
                ...
                train, val, test = [
                    list(arr)
                    for arr in np.split(subcat_img_paths, split_indices)
                ]
