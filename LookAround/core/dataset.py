...
from LookAround.utils import not_none_validator


@attr.s(auto_attribs=True, kw_only=True)
class Episode:
    r"""
    :property episode_id: id of the episode (i.e., episode number)
    :property video_id: file name `/data/pano_datasets/initialization/{scene}/R0010***` (without the extension)
    :property scene_id: scene type of the file (i.e., `aist_hall`)
    :property parent_scene_id: parent scene name (i.e., `outdoor`, `indoor`)
    :property start_rotation: rotation in radians [yaw, pitch, roll]
    :property info: other information
    """
    episode_id: str = attr.ib(default=None, validator=not_none_validator)
    video_id: str = attr.ib(default=None, validator=not_none_validator)
    scene_id: str = attr.ib(default=None, validator=not_none_validator)
    parent_scene_id: str = None
    start_position: int = attr.ib(default=None, validator=not_none_validator)
...
class Dataset(Generic[T]):
    ...
    def video_ids(self) -> List[str]:
        r"""unique video ids present in the dataset.
        """
        return sorted(list({episode.video_id for episode in self.episodes}))
    ...
    def get_video_episodes(self, video_id: str) -> List[T]:
        r"""..
        :param video_id: id of video in video dataset.
        "return: list of episodes for the :p: `video_id`.
        """
        return list(
            filter(lambda x: x.video_id == video_id, iter(self.episodes))
        )
    ...
    def filter_episodes(self, filter_fn: Callable[[T], bool]) -> "Dataset":
        ...
        new_dataset = copy.copy(self)  # FIXME: deepcopy?
    ...
    def get_splits(
        ...
        :param collate_video_ids: if true, episodes with the same video id are
            next to each other. This saves on overhead of switching between
            videos, but means multiple sequential episodes will be related to
            each other because they will be in the same video.
        ...
        if collate_video_ids:
            ...
            for rand_ind in rand_items:
                video = self.episodes[rand_ind].video_id
                if video not in video_ids:
                    video_ids[video] = []
                video_ids[video].append(rand_ind)
            rand_items = []
            list(map(rand_items.extend, video_ids.values()))
        ep_ind = 0
        new_episodes = []
...
class EpisodeIterator(Iterator):
    ...
    between videos, therefore episodes of the same video should be loaded
    consecutively. However, if too many consecutive episodes from same video
    are feed into RL model, the model will risk to overfit that video.
    Therefore it's better to load same video consecutively and switch once a
    ...
    Cycling with shuffle:
        when cycling back, shuffle episodes groups grouped by video.
    Group by video:
        episodes of same video will be grouped and loaded consecutively.
    Set episode repeat:
        set a number threshold on how many episodes from the same video can be
    ...
    def __init__(
        ...
        :param episode_repeat: how many episodes from the same episode
            is loaded consecutively. :py:`1` for repeat
        ...
        self.num_episode_repeat = episode_repeat
        self.shuffle = shuffle
        self._rep_count = 0
        self._prev_episode = None
        self._scene_rep_count = 0
    ...
    def __next__(self):
        ...
        if self._rep_count == 0:
            ...
            if self._prev_scene_id == next_episode.scene_id:
                self._scene_rep_count += 1
            if (
                self.max_scene_repetition > 0
                and self._scene_rep_count >= self.max_scene_repetition - 1
            ):
                self._shuffle_iterator()
                self._scene_rep_count = 0
            ...
            self._prev_episode = next_episode
        else:
            next_episode = self._prev_episode

        self._rep_count += 1
        if self._rep_count >= self.num_episode_repeat:
            self._rep_count = 0
