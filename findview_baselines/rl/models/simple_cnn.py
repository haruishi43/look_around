...
class SimpleCNN(nn.Module):
    ...
    def forward(self, observations: Dict[str, torch.Tensor]):
        cnn_input = []
        if self._n_input_pers > 0:
            pers_observations = observations["pers"]
            # NOTE: Already in chw
            # permute tensor to dimension [BATCH x CHANNEL x HEIGHT X WIDTH]
            # pers_observations = pers_observations.permute(0, 3, 1, 2)
            # pers_observations = (
            #     pers_observations.float() / 255.0
            # )  # normalize RGB
            cnn_input.append(pers_observations)
        if self._n_input_target > 0:
            target_observations = observations["target"]
            cnn_input.append(target_observations)
