...
class StaticMeanVar(nn.Module):
    def __init__(
        ...
        self.mean = mean
        self.var = var
        self.normalize = TVNormalize(mean, var)
...
class RunningMeanAndVar(nn.Module):
    def __init__(self, n_channels: int) -> None:
        ...
        # when using buffers, the parameters can be restored by `state_dict`,
        # but the buffers are not trained by the optimizer.
